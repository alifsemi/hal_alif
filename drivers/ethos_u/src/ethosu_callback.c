...
static bool check_need_to_invalidate_or_flush(const void *p, size_t bytes)
{
	...
	for (unsigned int i = 0;
	     i < sizeof no_need_to_invalidate_or_flush_areas / sizeof no_need_to_invalidate_or_flush_areas[0]; i++) {
...
bool __attribute__((weak)) ethosu_area_needs_invalidate_dcache(const void *p, size_t bytes)
{
	...
	return check_need_to_invalidate_or_flush(p, bytes);
}
