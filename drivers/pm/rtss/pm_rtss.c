...
static inline void save_fp_state(struct fp_state_t *state)
{
	__asm__ volatile(
		"VSTM %0, {D8-D15}\n\t"
		"VSTR FPSCR, [%0, #32]\n\t"
...
static inline void restore_fp_state(const struct fp_state_t *state)
{
	__asm__ volatile(
		"VLDM %0, {D8-D15}\n\t"
		"VLDR FPSCR, [%0, #32]\n\t"
...
void pm_core_enter_deep_sleep(void)
{
	...
	if (_FLD2VAL(PWRMODCTL_CPDLPSTATE_CLPSTATE, old_cpdlpstate)
		> PM_LPSTATE_ON_CLK_OFF) {
		PWRMODCTL->CPDLPSTATE = (old_cpdlpstate &
					~PWRMODCTL_CPDLPSTATE_CLPSTATE_Msk) |
					_VAL2FLD(PWRMODCTL_CPDLPSTATE_CLPSTATE,
					PM_LPSTATE_ON_CLK_OFF);
	}
...
void pm_core_enter_deep_sleep_request_subsys_off(void)
{
	...
	if (!(orig_cppwr & ICB_CPPWR_SU10_Msk)) {
		/* As we are going to say it's okay to lose EPU state, we should save
		 * it;we can't independently turn EPU off on our silicon, but the CPU
	...
	if (!(orig_cppwr & ICB_CPPWR_SU10_Msk)) {
		 ...
		 * anything that prompts the compiler to generate MVE/FP code during
		 * this function.
		 */
	...
	if (orig_mscr & MEMSYSCTL_MSCR_DCACTIVE_Msk) {
		...
		if (!(MEMSYSCTL->MSCR & MEMSYSCTL_MSCR_DCCLEAN_Msk)) {
			...
			/* Should be good to manually mark clean now - M55 TRM tells us not
			 * to, but after some discussion with Arm, I believe we've taken
			 * enough care that this is valid at this point.
...
void pm_core_request_subsys_off_from_spurious_wakeup(void)
{
	...
	/* Trigger the EWIC sleep - may or may not return*/
	/* Set up WICCONTROL so that deep sleep is the required WIC sleep type */
	regval = _VAL2FLD(WICCONTROL_WIC, 1) |
			_VAL2FLD(WICCONTROL_IWIC, PM_WIC_IS_EWIC);
	sys_write32(regval, WICCONTROL);
...
{
	/* Read the set bits */
	return sys_read32(RESET_STATUS_REG);
}
...
bool pm_core_wakeup_is_spurious(void)
{
	...
	if (pm_peek_subsystem_reset_status() != 0) {
		return false;
	}
	 ...
	 */
	if (!(sys_read32(WICCONTROL) & WICCONTROL_WIC_Msk)) {
		return false;
	}
 ...
 *           Application should take care of enabling the EWIC mask before
...
#if PM_RTSS_HANDLE_SPURIOUS_WAKEUP
/**
 * @fn       void pm_handle_spurious_wakeup(void)
 * @brief    Handle the spurious wakeup
 *           See if the core booted due to spurious wakeup.
 *           If yes, try requesting subsystem_off.
 * @return   None
 */
void __weak pm_handle_spurious_wakeup(void)
{
	if (pm_core_wakeup_is_spurious()) {
		pm_core_enable_manual_pd_sequencing();
		pm_core_request_subsys_off_from_spurious_wakeup();
...
#endif /* PM_RTSS_HANDLE_SPURIOUS_WAKEUP */
