...
static void usbd_dwc3_InitializeEps(USB_DRIVER *drv)
{
	...
	/* required in zephyr  */
	for (i = 0; i < (drv->NumOutEps + drv->NumInEps); i++) {
		memset(&drv->eps[i].EpTrb[0], 0x00, USB_TRBS_PER_EP * USB_TRB_STRUCTURE_SIZE);
...
static void usbd_dwc3_set_incr_burst_type(USB_DRIVER *drv)
{
	...
	reg |= USB_GSBUSCFG0_INCRBRSTENA;
	reg |= USB_GSBUSCFG0_INCR16BRSTENA;
...
static uint32_t usbd_dwc3_device_init(USB_DRIVER *drv)
{
	...
	drv->NumInEps = USB_IN_EPS(reg);
	drv->NumOutEps = (USB_NUM_EPS(reg) - drv->NumInEps);
	...
	reg = drv->regs->DEVTEN;
	reg |= USB_DEV_DISSCONNEVTEN;
	reg |= USB_DEV_USBRSTEVTEN;
	reg |= USB_DEV_CONNECTDONEEVTEN;
	drv->regs->DEVTEN = reg;
